...
def get_latest_security_update():
    ...
    if response.status_code == 200:
        ...
        ul_list = re.findall('The latest version of macOS is (.+?)\.&nbsp;Learn', result)
        if len(ul_list) == 0 or (len(ul_list) == 1 and ul_list[0].find('and') != -1):
            ul_list = re.findall('The latest version of macOS is (.+?), and', result)
        if len(ul_list) == 0:
            print("Unable to parse ul_list.")
        latest_ver = ul_list[0]
        link_no_cve_re = '<td>macOS [a-zA-Z ]* {0}[a-zA-Z0-9,.\\-_ ]*<br>\\s+<span class="note">(.+?)</span></td>'.format(latest_ver)
        link_cve_re = '<td><a href="(.+?)">macOS [a-zA-Z ]* {0}[a-zA-Z0-9,.\\-_ ]*</a></td>'.format(latest_ver)
